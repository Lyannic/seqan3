...
#include <seqan3/range/views/kmer_issh_naive_hash.hpp>
...
static void seqan_kmer_delete_mask_hash_ungapped(benchmark::State & state)
{
    ...
    for (auto _ : state)
    {
        for (auto h : seq | seqan3::views::kmer_delete_mask_hash(seqan3::ungapped{static_cast<uint8_t>(k)}))
            benchmark::DoNotOptimize(sum += h);
    }
...
static void seqan_kmer_nthash_ungapped(benchmark::State & state)
{
    ...
    for (auto _ : state)
    {
        for (auto h : seq | seqan3::views::kmer_nthash(seqan3::ungapped{static_cast<uint8_t>(k)}))
            benchmark::DoNotOptimize(sum += h);
    }
...
static void nthash_ungapped(benchmark::State & state)
{
    ...
    size_t k = static_cast<size_t>(state.range(1));
    assert(k > 0);
    auto generatedSeq = seqan3::test::generate_sequence<seqan3::dna4>(sequence_length, 0, 0);
    auto genereatedSeqVec = generatedSeq | seqan3::views::to_char;
...
static void seqan_kmer_gapped_hash_gapped(benchmark::State & state)
{
    ...
    for (auto _ : state)
    {
        for (auto h : seq | seqan3::views::kmer_gapped_hash(make_gapped_shape(k)))
            benchmark::DoNotOptimize(sum += h);
    }
...
static void seqan_kmer_gapped_delete_mask_hash_gapped(benchmark::State & state)
{
    ...
    for (auto _ : state)
    {
        for (auto h : seq | seqan3::views::kmer_gapped_delete_mask_hash(make_gapped_shape(k)))
            benchmark::DoNotOptimize(sum += h);
    }
...
static void seqan_kmer_gapped_perfect_hash_gapped(benchmark::State & state)
{
    ...
    for (auto _ : state)
    {
        for (auto h : seq | seqan3::views::kmer_gapped_perfect_hash(make_gapped_shape(k)))
            benchmark::DoNotOptimize(sum += h);
    }
...
static void seqan_kmer_gapped_delete_mask_perfect_hash_gapped(benchmark::State & state)
{
    ...
    for (auto _ : state)
    {
        for (auto h : seq | seqan3::views::kmer_gapped_delete_mask_perfect_hash(make_gapped_shape(k)))
            benchmark::DoNotOptimize(sum += h);
    }
...
static void seqan_kmer_issh_hash_gapped(benchmark::State & state)
{
    ...
    for (auto _ : state)
    {
        for (auto h : seq | seqan3::views::kmer_issh_hash(shape, spaced_qmer.GetMultipleShifts(), spaced_qmer.GetPosOne()))
            benchmark::DoNotOptimize(sum += h);
    }
...
static void seqan_kmer_issh_hash_gapped_with_precomputations(benchmark::State & state)
{
    ...
    for (auto _ : state)
    {
        for (auto h : seq | seqan3::views::kmer_issh_precomp_hash(make_gapped_shape(k)))
            benchmark::DoNotOptimize(sum += h);
    }
...
static void seqan_kmer_issh_naive_hash_gapped(benchmark::State & state)
{
    auto sequence_length = state.range(0);
    assert(sequence_length > 0);
    size_t k = static_cast<size_t>(state.range(1));
    ...
    for (auto _ : state)
    {
        for (auto h : seq | seqan3::views::kmer_issh_naive_hash(make_gapped_shape(k)))
            benchmark::DoNotOptimize(sum += h);
    }
...
BENCHMARK(seqan_kmer_issh_naive_hash_gapped)->Apply(arguments);
