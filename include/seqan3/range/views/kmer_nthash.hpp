 ...
 * \brief Provides seqan3::views::kmer_nthash.
 */
...
{
// ---------------------------------------------------------------------------------------------------------------------
// kmer_nthash_view class
// ---------------------------------------------------------------------------------------------------------------------

/*!\brief The type returned by seqan3::views::kmer_nthash.
...
class kmer_nthash_view : public std::ranges::view_interface<kmer_nthash_view<urng_t>>
{
private:
    static_assert(std::ranges::forward_range<urng_t const>, "The kmer_nthash_view only works on forward_ranges");
...
public:
     ...
     * \{
     */
    kmer_nthash_view()                                       = default; //!< Defaulted.
    kmer_nthash_view(kmer_nthash_view const & rhs)             = default; //!< Defaulted.
    kmer_nthash_view(kmer_nthash_view && rhs)                  = default; //!< Defaulted.
    kmer_nthash_view & operator=(kmer_nthash_view const & rhs) = default; //!< Defaulted.
    kmer_nthash_view & operator=(kmer_nthash_view && rhs)      = default; //!< Defaulted.
    ~kmer_nthash_view()                                      = default; //!< Defaulted.
    ...
    template <typename rng_t>
    //!\cond
     requires !std::same_as<remove_cvref_t<rng_t>, kmer_nthash_view> &&
...
template <std::ranges::view urng_t>
template <typename rng_t>
class kmer_nthash_view<urng_t>::shape_iterator
{
private:
...
private:
    ...
    void hash_roll_forward()
    {
        ...
        hash_value = seqanNTF64(hash_value, shape_.size(), to_rank(*(text_left)), to_rank(*(text_right)));
...
kmer_nthash_view(rng_t &&, shape const & shape_) -> kmer_nthash_view<std::ranges::all_view<rng_t>>;

// ---------------------------------------------------------------------------------------------------------------------
// kmer_nthash_fn (adaptor definition)
// ---------------------------------------------------------------------------------------------------------------------

//![adaptor_def]
//!\brief views::kmer_nthash's range adaptor object type (non-closure).
struct kmer_nthash_fn
{
    ...
    constexpr auto operator()(urng_t && urange, shape const & shape_) const
    {
        static_assert(std::ranges::viewable_range<urng_t>,
            "The range parameter to views::kmer_nthash cannot be a temporary of a non-view range.");
        static_assert(std::ranges::forward_range<urng_t>,
            "The range parameter to views::kmer_nthash must model std::ranges::forward_range.");
        static_assert(semialphabet<std::ranges::range_reference_t<urng_t>>,
            "The range parameter to views::kmer_nthash must be over elements of seqan3::semialphabet.");

        return kmer_nthash_view{std::forward<urng_t>(urange), shape_};
    }
};
//![adaptor_def]
 ...
 * ### Example
 *
 * \include test/snippet/range/views/kmer_nthash.cpp
 *
 * \hideinitializer
 */
inline constexpr auto kmer_nthash = detail::kmer_nthash_fn{};
