 ...
 * \brief Provides seqan3::views::kmer_issh_hash_with_precomputations.
 */
...
// ---------------------------------------------------------------------------------------------------------------------
// kmer_issh_hash_with_precomputations_view class
// ---------------------------------------------------------------------------------------------------------------------

/*!\brief The type returned by seqan3::views::kmer_issh_hash_with_precomputations.
...
class kmer_issh_hash_with_precomputations_view : public std::ranges::view_interface<kmer_issh_hash_with_precomputations_view<urng_t>>
{
private:
    static_assert(std::ranges::forward_range<urng_t const>, "The kmer_issh_hash_with_precomputations_view only works on forward_ranges");
    ...
    public:
        ...
        */
            ...
            shape_{s_}, text_left{it_start}, text_right{it_start}
        {
            ...
            {
                sequence += (*text_right).to_char();
                std::ranges::advance(text_right, 1);
            }
            // seqan3::debug_stream << "sequence: " << sequence << "\n";
            ...
            V_shifts = spaced_qmer.GetMultipleShifts();
            pos_one = spaced_qmer.GetPosOne();
            seqan3::debug_stream << "sequence: " << sequence << "\n";
            hash_full();
    ...
    private:
        ...
        void hash_gapped()
        {
            ...
            {
                if (hash_count < V_shifts.size()) 
                {
                    if (V_shifts[hash_count].size() == 0)
                    {
                        hash_full_gapped();
                    }
                    else
                    {
                        hash_issh(V_shifts[hash_count]);
                        std::ranges::advance(text_left,  1);
                    }
                }
                else
                {
                    hash_issh(V_shifts[0]);
                    std::ranges::advance(text_left,  1);
                }
            }
        ...
        void hash_issh(const V_PreviusShift& shifts)
        {
            ...
            for (size_t j = 0; j < shifts[0].one_to_change.size(); ++j)
            {
                ...
                size_t as_rank;
                if(sequence[index_char] == 'A') 
                {
                    as_rank = 0;
                } else if (sequence[index_char] == 'C')
                {
                    as_rank = 1;
                } else if (sequence[index_char] == 'G')
                {
                    as_rank = 2;
                } else 
                {
                ...
                hash_value |= as_rank << ((count_relevant_positions - 1 - shifts[0].one_to_change[j]) * 2);
                // seqan3::debug_stream << "as_rank: " << as_rank << "\n";
            }
            hash_values.push_back(hash_value);
            seqan3::debug_stream << "rolling" << "\n";
        }
...
public:
     ...
     * \{
     */
    kmer_issh_hash_with_precomputations_view()                                       = default; //!< Defaulted.
    kmer_issh_hash_with_precomputations_view(kmer_issh_hash_with_precomputations_view const & rhs)             = default; //!< Defaulted.
    kmer_issh_hash_with_precomputations_view(kmer_issh_hash_with_precomputations_view && rhs)                  = default; //!< Defaulted.
    kmer_issh_hash_with_precomputations_view & operator=(kmer_issh_hash_with_precomputations_view const & rhs) = default; //!< Defaulted.
    kmer_issh_hash_with_precomputations_view & operator=(kmer_issh_hash_with_precomputations_view && rhs)      = default; //!< Defaulted.
    ~kmer_issh_hash_with_precomputations_view()                                      = default; //!< Defaulted.
     ...
     */
    kmer_issh_hash_with_precomputations_view(urng_t urange_, shape const & s_) : urange{std::move(urange_)}, shape_{s_}
    {
    ...
    template <typename rng_t>
    //!\cond
     requires !std::same_as<remove_cvref_t<rng_t>, kmer_issh_hash_with_precomputations_view> &&
              std::ranges::viewable_range<rng_t> &&
              std::constructible_from<urng_t, ranges::ref_view<std::remove_reference_t<rng_t>>>
    //!\endcond
    kmer_issh_hash_with_precomputations_view(rng_t && urange_, shape const & s_) :
...
kmer_issh_hash_with_precomputations_view(rng_t &&, shape const & shape_) -> kmer_issh_hash_with_precomputations_view<std::ranges::all_view<rng_t>>;

// ---------------------------------------------------------------------------------------------------------------------
// kmer_issh_hash_with_precomputations_fn (adaptor definition)
// ---------------------------------------------------------------------------------------------------------------------

//![adaptor_def]
//!\brief views::kmer_issh_hash_with_precomputations's range adaptor object type (non-closure).
struct kmer_issh_hash_with_precomputations_fn
{
    ...
    constexpr auto operator()(urng_t && urange, shape const & shape_) const
    {
        static_assert(std::ranges::viewable_range<urng_t>,
            "The range parameter to views::kmer_issh_hash_with_precomputations cannot be a temporary of a non-view range.");
        static_assert(std::ranges::forward_range<urng_t>,
            "The range parameter to views::kmer_issh_hash_with_precomputations must model std::ranges::forward_range.");
        static_assert(semialphabet<reference_t<urng_t>>,
            "The range parameter to views::kmer_issh_hash_with_precomputations must be over elements of seqan3::semialphabet.");

        return kmer_issh_hash_with_precomputations_view{std::forward<urng_t>(urange), shape_};
    }
};
//![adaptor_def]
 ...
 * ### Example
 *
 * \include test/snippet/range/views/kmer_issh_hash_with_precomputations.cpp
 *
 * \hideinitializer
 */
inline constexpr auto kmer_issh_hash_with_precomputations = detail::kmer_issh_hash_with_precomputations_fn{};
