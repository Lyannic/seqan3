 ...
 * \brief Provides seqan3::views::kmer_delete_mask_hash.
 */
...
{
// ---------------------------------------------------------------------------------------------------------------------
// kmer_delete_mask_hash_view class
// ---------------------------------------------------------------------------------------------------------------------

/*!\brief The type returned by seqan3::views::kmer_delete_mask_hash.
...
class kmer_delete_mask_hash_view : public std::ranges::view_interface<kmer_delete_mask_hash_view<urng_t>>
{
private:
    static_assert(std::ranges::forward_range<urng_t const>, "The kmer_delete_mask_hash_view only works on forward_ranges");
...
public:
     ...
     * \{
     */
    kmer_delete_mask_hash_view()                                       = default; //!< Defaulted.
    kmer_delete_mask_hash_view(kmer_delete_mask_hash_view const & rhs)             = default; //!< Defaulted.
    kmer_delete_mask_hash_view(kmer_delete_mask_hash_view && rhs)                  = default; //!< Defaulted.
    kmer_delete_mask_hash_view & operator=(kmer_delete_mask_hash_view const & rhs) = default; //!< Defaulted.
    kmer_delete_mask_hash_view & operator=(kmer_delete_mask_hash_view && rhs)      = default; //!< Defaulted.
    ~kmer_delete_mask_hash_view()                                      = default; //!< Defaulted.
     ...
     */
    kmer_delete_mask_hash_view(urng_t urange_, shape const & s_) : urange{std::move(urange_)}, shape_{s_}
    {
    ...
    template <typename rng_t>
    //!\cond
     requires !std::same_as<remove_cvref_t<rng_t>, kmer_delete_mask_hash_view> &&
              std::ranges::viewable_range<rng_t> &&
              std::constructible_from<urng_t, ranges::ref_view<std::remove_reference_t<rng_t>>>
    //!\endcond
    kmer_delete_mask_hash_view(rng_t && urange_, shape const & s_) :
...
template <std::ranges::view urng_t>
template <typename rng_t>
class kmer_delete_mask_hash_view<urng_t>::shape_iterator
{
private:
...
kmer_delete_mask_hash_view(rng_t &&, shape const & shape_) -> kmer_delete_mask_hash_view<std::ranges::all_view<rng_t>>;

// ---------------------------------------------------------------------------------------------------------------------
// kmer_delete_mask_hash_fn (adaptor definition)
// ---------------------------------------------------------------------------------------------------------------------

//![adaptor_def]
//!\brief views::kmer_delete_mask_hash's range adaptor object type (non-closure).
struct kmer_delete_mask_hash_fn
{
    ...
    constexpr auto operator()(urng_t && urange, shape const & shape_) const
    {
        static_assert(std::ranges::viewable_range<urng_t>,
            "The range parameter to views::kmer_delete_mask_hash cannot be a temporary of a non-view range.");
        static_assert(std::ranges::forward_range<urng_t>,
            "The range parameter to views::kmer_delete_mask_hash must model std::ranges::forward_range.");
        static_assert(semialphabet<std::ranges::range_reference_t<urng_t>>,
            "The range parameter to views::kmer_delete_mask_hash must be over elements of seqan3::semialphabet.");

        return kmer_delete_mask_hash_view{std::forward<urng_t>(urange), shape_};
    }
};
//![adaptor_def]
 ...
 * ### Example
 *
 * \include test/snippet/range/views/kmer_delete_mask_hash.cpp
 *
 * \hideinitializer
 */
inline constexpr auto kmer_delete_mask_hash = detail::kmer_delete_mask_hash_fn{};
