 ...
 * \brief Provides seqan3::views::kmer_issh_hash.
 */
...
{
// ---------------------------------------------------------------------------------------------------------------------
// kmer_issh_hash_view class
// ---------------------------------------------------------------------------------------------------------------------

/*!\brief The type returned by seqan3::views::kmer_issh_hash.
...
class kmer_issh_hash_view : public std::ranges::view_interface<kmer_issh_hash_view<urng_t>>
{
private:
    static_assert(std::ranges::forward_range<urng_t const>, "The kmer_issh_hash_view only works on forward_ranges");
...
public:
     ...
     * \{
     */
    kmer_issh_hash_view()                                       = default; //!< Defaulted.
    kmer_issh_hash_view(kmer_issh_hash_view const & rhs)             = default; //!< Defaulted.
    kmer_issh_hash_view(kmer_issh_hash_view && rhs)                  = default; //!< Defaulted.
    kmer_issh_hash_view & operator=(kmer_issh_hash_view const & rhs) = default; //!< Defaulted.
    kmer_issh_hash_view & operator=(kmer_issh_hash_view && rhs)      = default; //!< Defaulted.
    ~kmer_issh_hash_view()                                      = default; //!< Defaulted.
    ...
    template <typename rng_t>
    //!\cond
     requires !std::same_as<remove_cvref_t<rng_t>, kmer_issh_hash_view> &&
    ...
    auto begin() noexcept
    {
        return shape_iterator<urng_t>{std::ranges::begin(urange), std::ranges::end(urange), shape_, V_shifts, pos_one};
    }
    ...
    //!\endcond
    {
        return shape_iterator<urng_t const>{std::ranges::begin(urange), std::ranges::end(urange), shape_, V_shifts, pos_one};
    }
...
template <std::ranges::view urng_t>
template <typename rng_t>
class kmer_issh_hash_view<urng_t>::shape_iterator
{
private:
...
public:
    ...
    */
    shape_iterator(it_t it_start, sentinel_t it_end, shape s_, V_V_PreviusShift const & _V_shifts, Position const & _pos_one) :
        shape_{s_}, V_shifts(_V_shifts), pos_one(_pos_one), text_left{it_start}, text_right{std::ranges::next(it_start, s_.size(), it_end)}
    {
...
kmer_issh_hash_view(rng_t &&, shape const & shape_, V_V_PreviusShift const & V_shifts, Position const & pos_one) -> kmer_issh_hash_view<std::ranges::all_view<rng_t>>;
...
//![adaptor_def]
//!\brief views::kmer_issh_hash's range adaptor object type (non-closure).
struct kmer_issh_hash_fn
{
    ...
    constexpr auto operator()(urng_t && urange, shape const & shape_, V_V_PreviusShift const & V_shifts, Position const & pos_one) const
    {
        ...
        static_assert(std::ranges::forward_range<urng_t>,
            "The range parameter to views::kmer_issh_hash must model std::ranges::forward_range.");
        static_assert(semialphabet<std::ranges::range_reference_t<urng_t>>,
 ...
 * ### Example
 *
 * \include test/snippet/range/views/kmer_issh_hash.cpp
 *
 * \hideinitializer
 */
inline constexpr auto kmer_issh_hash = detail::kmer_issh_hash_fn{};
